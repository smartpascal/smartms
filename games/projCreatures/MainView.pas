...
uses 
  FireSpriteAtlas, W3C.Console, W3C.HTML5, ECMA.JSON, System.Types, SmartCL.System,
  SmartCL.Graphics, SmartCL.Components, SmartCL.Time;
...
type
  TAnimation = record
    ...
    published
      ...
      function getCurrentFrame: variant;
      class function create(name: String; FrameNames: Variant; framesRate: Integer): TAnimation; static;
    end;

type
  TMainView = class(TW3GraphicControl)
  private
    FDelay: Integer;
    FTimer: TW3EventRepeater;
    animObj : TFireSpriteAtlas;
    BackgroundA, BackgroundB: JHTMLImageElement;
    CurrFrame : Integer;
    animations: array of TAnimation;

    function HandleCallBack(Sender: TObject): Boolean;
  ...
  public
    property Speed: Integer read FDelay write SetDelay;
    procedure Start;
    procedure Stop;
    procedure Toggle;
  end;
...
procedure TMainView.InitializeObject;
begin
  inherited;
  FDelay := 1000 div FRAMERATE;
  w3_setStyle(Handle, 'backgroundColor', '#ffffff');

  (* create the Animation instance *)
  animObj := new TFireSpriteAtlas( JSON.parse(JSON_DATA) );

  (* create the background sea images *)
	BackgroundA := JHTMLImageElement(document.createElement('img'));
  BackgroundA.src := "res/undersea.jpg";
	BackgroundB := JHTMLImageElement(document.createElement('img'));
  BackgroundB.src := "res/seabed.png";

  (* create individual animations *)
  animations.Add( TAnimation.create('jellyfish',      genFrameNames('blueJellyfish',  0, 32, '', 4), 30) );
  animations.Add( TAnimation.create('crab',           genFrameNames('crab1',          0, 25, '', 4), 30) );
  animations.Add( TAnimation.create('greenJellyfish', genFrameNames('greenJellyfish', 0, 39, '', 4), 30) );
  animations.Add( TAnimation.create('octopus',        genFrameNames('octopus',        0, 24, '', 4), 30) );
  animations.Add( TAnimation.create('purpleFish',     genFrameNames('purpleFish',     0, 20, '', 4), 30) );
  animations.Add( TAnimation.create('seahorse',       genFrameNames('seahorse',       0, 5,  '', 4), 30) );
  animations.Add( TAnimation.create('stingray',       genFrameNames('stingray',       0, 23, '', 4), 30) );
end;

procedure TMainView.FinalizeObject;
begin
...
procedure TMainView.SetDelay(AValue: Integer);
begin
...
function TMainView.HandleCallBack(Sender: TObject): Boolean;
begin
  Result := False;
  Invalidate;
end;
...
begin
  if not Assigned(FTimer) then
    FTimer := TW3EventRepeater.Create(HandleCallBack, FDelay);
end;
...
procedure TMainView.Paint;
var
  Wd, Hd:  Integer;

begin
  inherited;
  (* Get viewport size and determine center *)
  Wd := Width;
  Hd := Height;
  ...
  var ctx := DC;
  (* Clear background  *)
  ctx.FillStyle := 'rgb(0, 0, 99)';
  ctx.clearRect(0, 0, Wd, Hd);
  (* draw the sea background *)
  ctx.drawImage(BackgroundA, 0, 0);
	ctx.drawImage(BackgroundB, 0, 466);

var JellyfishanimObj, CrabanimObj, GreenJellyfishanimObj, OctopusanimObj, PurpleFishanimObj, SeahorseanimObj, StingrayanimObj,  squidFirstanimObj, FlyingFishFirstanimObj   : variant;
  (* create sprite pictures *)
	JellyfishanimObj       := animObj.createSprite( animations[0].getCurrentFrame);
	CrabanimObj            := animObj.createSprite( animations[1].getCurrentFrame);
	GreenJellyfishanimObj  := animObj.createSprite( animations[2].getCurrentFrame);
	OctopusanimObj         := animObj.createSprite( animations[3].getCurrentFrame);
	PurpleFishanimObj      := animObj.createSprite( animations[4].getCurrentFrame);
	SeahorseanimObj        := animObj.createSprite( animations[5].getCurrentFrame);
	StingrayanimObj        := animObj.createSprite( animations[6].getCurrentFrame);
	squidFirstanimObj      := animObj.createSprite('squid0000');
	FlyingFishFirstanimObj := animObj.createSprite('flyingFish0000');

var PurpleFishANI, PurpleFishReturnANI, OctopusANI, OctopusReturnANI, GreenJellyfishANI, GreenJellyfishReturnANI, JellyfishANI : Float;
  (* apply animation tween *)
	PurpleFishANI           := ExpoInOut(currFrame, 1000, -1200, FRAMERATE);
	PurpleFishReturnANI     := ExpoInOut(currFrame, 530, -580, FRAMERATE);
	OctopusANI              := ExpoInOut(currFrame, 530, -PurpleFishReturnANI, FRAMERATE);
	OctopusReturnANI        := ExpoInOut(currFrame, 350, -350, FRAMERATE);
	GreenJellyfishANI       := ExpoInOut(currFrame, 350, -OctopusReturnANI, FRAMERATE);
	GreenJellyfishReturnANI := ExpoInOut(currFrame, 180, -180, FRAMERATE);
	JellyfishANI            := ExpoInOut(currFrame, 100, -GreenJellyfishReturnANI, FRAMERATE);

  (* drawing the pictures *)
	JellyfishanimObj.draw(ctx, 670, 20);
	CrabanimObj.draw(ctx, 550, 480);
	JellyfishanimObj.draw(ctx, 670, JellyfishANI);
	GreenJellyfishanimObj.draw(ctx, 330, GreenJellyfishANI);
	OctopusanimObj.draw(ctx, 160, OctopusANI);
	PurpleFishanimObj.draw(ctx, PurpleFishANI, 413);
	SeahorseanimObj.draw(ctx, 491, 40);
	StingrayanimObj.draw(ctx, 80, 190);
	squidFirstanimObj.draw(ctx, 610, 215);
	FlyingFishFirstanimObj.draw(ctx, 60, 40);

  (* update the current Frame *)
  currFrame := if(currFrame < FRAMERATE) then currFrame + 1 else 0;
